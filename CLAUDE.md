...
│       └── tester/SKILL.md
├── docs/
│   ├── table_description.md     # table-level doc blocks
│   └── column_decription.md     # column-level doc blocks (filename spelling is load-bearing)
├── seeds/
│   ├── raw_machines.csv
│   ├── raw_event_types.csv
│   ├── raw_error_codes.csv
...
reproducible. The dataset is deliberately **small and fast to build** so the whole
project can be rebuilt live during a presentation. **Hard cap: total rows across all
seed files must stay under 10,500.**
...
| `raw_device_events.csv` | 1 row per emitted event          | **~10,100** | fact — see below |
| **Total**                |                                  | **~10,148** | comfortably under the 10,500 cap |
...
| `load_date` | date | **which daily load batch delivered this row** — derived from `source_ingested_at`; the key that lets the demo replay one day at a time |

### 3.5 Generation rules

- **Time span:** exactly 5 days (`2026-04-01` through `2026-04-05`), modelled as
  **five daily load batches**. Every row carries a `load_date` naming the batch that
  delivered it; the ~10,000 events are distributed across the five dates.
- **Volume target:** **~10,000 events total** across 20 machines × 5 days
  (~100 events/machine/day). Tune per-event frequencies until the total lands in the
  9,500–10,500 range. Small volume is a hard requirement: the demo must rebuild
  end-to-end in seconds, not minutes.
...
- **Per-event relative frequency (guideline weights, tune to hit volume target):**
  ...
  - `STATUS_CHANGE`: low frequency — target **5–9 transitions per machine over the
    full 5 days**, cycling through `RUNNING → IDLE → RUNNING → MAINTENANCE → RUNNING → ERROR → RUNNING → STOPPED …`. This is what feeds the silver SCD2 model, so it must be sparse but present for every machine.
  - `ERROR_RAISED` / `ERROR_RESOLVED`: rare — 2–4 fault episodes per machine over the 5 days, always paired (a resolve follows a raise), referencing `raw_error_codes`.
...
- **Data quality noise (intentional, for the tests to catch / *silver* to clean):**
  - **~2% duplicate events**, re-delivered in the **following** daily batch: the same
    `event_id` reappears with `load_date + 1 day` and a later `source_ingested_at`.
    This mirrors a real at-least-once source that replays a slice of the previous
    day, and gives the silver dedup (`keep the latest-ingested copy`) something
    genuinely non-trivial to resolve.
  - **~0.5% rows with a null `machine_id`**, which silver quarantines.
  Bronze deliberately lands both untouched — the `not_null`/`unique` tests live at
  the **silver** layer and pass *because* of the silver transformation, not because
  the raw data was already clean.
...
### 4.2 Bronze layer — **incremental, append-only landing zone**

Every bronze model uses `materialized: incremental` with
`incremental_strategy='append'` and **no `unique_key`**. Bronze is a faithful
landing zone: it lands whatever the batch delivered, as delivered. **No
deduplication, no null filtering, no delta/watermark logic lives in bronze** —
all of that is the silver layer's responsibility. Re-running the same batch
legitimately appends the rows again; that duplication is expected and is exactly
what silver has to resolve.

Folder-level default in `dbt_project.yml`:
...
Models — each casts/standardizes types and adds a `_loaded_at` audit column, nothing
more:

- **`bronze_machines`** — `ref('raw_machines')`.
- **`bronze_event_types`** — `ref('raw_event_types')`.
- **`bronze_error_codes`** — `ref('raw_error_codes')`.
- **`bronze_device_events`** — `ref('raw_device_events')`, carrying `load_date`
  through, and filtered by the `load_date` project var so a single seed can be
  replayed one daily batch at a time:
...
**Demo payoff:** `dbt build --vars 'load_date: 2026-04-02'` simulates a single daily
scheduled run; running the five dates in sequence replays the whole ingestion history
batch by batch, and re-running one date shows bronze happily double-landing it while
silver's dedup collapses it back to one row per `event_id`. Omitting the var loads all
five batches at once. Incrementality one layer up is demonstrated by the
`agg_event_frequency_*` models' watermark-driven reads, which process zero rows on an
...
**Silver owns all data quality.** Because bronze is a blind append-only landing
zone (section 4.2), every silver model that reads bronze must:

1. **Quarantine** rows with a null `machine_id` (filtered out with a documented
   rationale — they cannot be attributed to a machine).
2. **Deduplicate events** by `event_id`, keeping the latest-ingested copy:
   `qualify row_number() over (partition by event_id order by source_ingested_at desc, _loaded_at desc) = 1`.
   This is what resolves the ~2% of events the source re-delivers in the following
   daily batch.
3. **Deduplicate dimension rows**, since bronze dimensions re-append the full seed
   on every run: `qualify row_number() over (partition by <business_key> order by _loaded_at desc) = 1`
   before joining. Models that only need distinct keys (`silver_tec_watermark`,
   `silver_tec_kpi_mapping`) use `select distinct` instead.
...
**Test placement follows the layer responsibilities**: because bronze is an
append-only landing zone that deliberately admits duplicates and null
`machine_id`s (section 4.2), **`unique` and `not_null`-on-`machine_id` tests do not
belong in bronze** — they are asserted at the **silver** layer, where they pass
because of the dedup/quarantine transformation. `relationships` tests are still
valid in bronze (dbt's implementation ignores null child values, and duplicate
parent rows are harmless for a referential check).

| Layer | Model | Tests |
|---|---|---|
| raw (seeds) | `raw_machines` | `unique`+`not_null` on `machine_id`; `accepted_values` on `machine_type` |
...
| raw (seeds) | `raw_device_events` | `not_null` on `event_id`, `event_type_code`, `event_timestamp`, `load_date`; `relationships` to `raw_event_types.event_type_code`; `accepted_values` on `status_value` (nulls excluded) and on `load_date` (the five simulated batch dates). **No** `unique` on `event_id` and **no** `not_null` on `machine_id` — the seed intentionally carries that noise |
| bronze | `bronze_machines` | `not_null` on `machine_id`; `accepted_values` on `machine_type` |
| bronze | `bronze_device_events` | `not_null` on `event_id`, `event_timestamp`, `load_date`, `_loaded_at`; `relationships` to `bronze_machines.machine_id` and `bronze_event_types.event_type_code` |
| silver | `silver_dim_machines` | `not_null` on `machine_sk`, `machine_id`, `valid_from`; `unique` on `machine_sk`; `dbt_utils.unique_combination_of_columns` on `machine_id`+`valid_from`; singular test asserting no overlapping `[valid_from, valid_to)` ranges per `machine_id`; singular test asserting exactly one `is_current = true` row per `machine_id` |
| silver | `silver_fact_device_events` | `unique`+`not_null` on `event_id`; `not_null` on `machine_id` (proves the quarantine works) and `load_date`; `relationships` to `silver_dim_machines.machine_id` |
| silver | `silver_fact_error_events` | `unique`+`not_null` on `error_event_sk`; `not_null` on `machine_id`, `error_code`, `raised_at`; `relationships` to `bronze_error_codes.error_code`; `dbt_expectations.expect_column_values_to_be_between` on `resolution_time_seconds` |
...
| silver | `silver_tec_batch_execution_log` | `not_null` on `batch_run_id`, `model_name`, `run_started_at`, `run_completed_at` |
...
- Every model gets a `.yml` doc entry with a `description` and at least a PK test.
- **Every column of every model and seed must have a description, and every
  description must be supplied through a doc block** — never inline prose in the
  `.yml`. Table-level blocks live in `docs/table_description.md`, column-level blocks
  in `docs/column_decription.md`, and are referenced as
  `description: "{{ doc('column_machine_id') }}"`. Doc-block names are global, so a
  column whose meaning differs by context needs a distinct block name (e.g.
  `column_model_name` for the machine model vs. `column_batch_model_name` for the
  batch log).
- `+persist_docs: {relation: true, columns: true}` is set project-wide for both
  models and seeds, so those descriptions are pushed into Unity Catalog and are
  queryable via `describe table extended` and `information_schema.tables` /
...
- Builds must run with `--threads 1`: the three `agg_event_frequency_*` models merge
  into the shared `silver_tec_watermark` table from their post-hooks, which conflicts
...
- `checks_agg_event_frequency.yml` — `event_count >= 1` for all rows;
  ...
  `bucket_start` values fall within the synthetic dataset's 5-day window.
...
2. Write `scripts/generate_synthetic_data.py` implementing section 3; run it to
   produce the four CSVs in `seeds/`; print per-`load_date` row counts and assert
   the <10,500 total-row cap.
3. `dbt seed` to load raw data.
4. Implement bronze models (section 4.2) — incremental `append`, no `unique_key`,
   no dedup, `load_date` var filter on `bronze_device_events`.
5. Implement silver business models (section 4.3) — quarantine + dedup logic,
   SCD2 window-function logic for `silver_dim_machines`; conformed fact tables
   `silver_fact_device_events` (incl. the `event_value` column) and
   `silver_fact_error_events`.
...
9. Write all YAML docs + tests (section 5) — every column described via a doc
   block; add `packages.yml` and run `dbt deps`.
10. Add the singular dbt test(s), including the no-overlapping-SCD2-periods check.
11. `dbt build --threads 1` end-to-end; confirm 0 failures.
12. Replay the five daily batches with `dbt build --vars 'load_date: <date>'`;
    confirm bronze appends only that batch, re-running a date double-lands it in
    bronze while silver still yields one row per `event_id`, and confirm the
    `agg_event_frequency_*` models' watermark-driven read processes 0 new rows on
    an unchanged second run (log this for the README).
...
16. After a `dbt build --full-refresh`, verify the persisted descriptions are
    queryable in Databricks (`describe table extended`,
    `information_schema.tables.comment`, `information_schema.columns.comment`).
17. Write a top-level `README.md` summarizing the showcase and the exact commands
    from section 6, including the daily-batch replay from step 12, the comment
    verification from step 16, and how to run the SODA scan from step 13.
